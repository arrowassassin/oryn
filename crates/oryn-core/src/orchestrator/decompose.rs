...
fn core_kind(lower: &str) -> SubtaskKind {
    if mentions(lower, &["bug", "fix", "broken", "fails", "failing", "race", "crash", "regression", "debug", "flaky"]) {
        SubtaskKind::Debugging
    } else if mentions(lower, &["refactor", "clean up", "cleanup", "restructure", "simplify", "extract", "decompose", "untangle"]) {
        SubtaskKind::Refactor
    } else if mentions(lower, &["rename", "move ", "reformat", "format", "bump", "version", "typo", "lint", "whitespace"]) {
...
pub fn decompose(mission_id: impl Into<String>, goal: &str) -> Mission {
    ...
    if mentions(&lower, &["across", "codebase", "whole repo", "entire", "audit", "everywhere", "all files", "every file", "throughout"]) {
    ...
    let core_summary = if goal.is_empty() { "Implement the requested change.".to_string() } else { goal.to_string() };
    subtasks.push(Subtask {
        id: core_id.clone(),
    ...
    if mentions(&lower, &["test", "spec", "coverage", "passing", "pass the", "tdd", "assert"]) {
        subtasks.push(Subtask {
    ...
    Mission { id: mission_id.into(), goal: goal.to_string(), subtasks }
}
...
mod tests {
    ...
    fn bug_goal_is_debugging_plus_tests() {
        ...
        let kinds: Vec<_> = m.subtasks.iter().map(|s| (s.id.as_str().to_string(), s.kind)).collect();
        ...
        let tests = m.subtasks.iter().find(|s| s.id.as_str() == "tests").unwrap();
    ...
    fn codebase_wide_goal_adds_leading_analysis() {
        let m = decompose("m", "Audit the entire codebase for unwrap() and replace across all files");
        ...
        let implement = m.subtasks.iter().find(|s| s.id.as_str() == "implement").unwrap();
    ...
    fn refactor_and_mechanical_verbs_route_correctly() {
        assert_eq!(decompose("m", "Refactor the auth client").subtasks.last().unwrap().kind, SubtaskKind::Refactor);
        assert_eq!(decompose("m", "Rename the helper module").subtasks.last().unwrap().kind, SubtaskKind::MechanicalEdit);
        assert_eq!(decompose("m", "Add a dark-mode toggle").subtasks.last().unwrap().kind, SubtaskKind::DiffEdit);
    }
