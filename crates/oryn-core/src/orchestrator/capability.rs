...
pub fn resolve_matrix(
    ...
    for kind in SubtaskKind::ALL {
        ...
        let mut candidates: Vec<(&ModelSpec, f64)> = available
            .iter()
            .filter_map(|spec| {
                let score = profiles.get(&spec.id).map(|p| p.score(kind)).unwrap_or(0.0);
...
mod tests {
    ...
    fn resolve_matrix_local_zero_cost_before_priced_api() {
        ...
        assert_eq!(
            tier[0],
            lt("local-ok"),
            "local/zero-cost must precede priced API"
        );
    ...
    fn resolve_matrix_empty_when_no_capable_models() {
        ...
        let matrix = resolve_matrix(&available, &profiles);
        assert!(
            matrix.tier(kind).is_empty(),
            "no capable model → empty tier"
        );
    }
    ...
    fn resolve_matrix_is_deterministic() {
        ...
        assert_eq!(
            m1, m2,
            "two calls with identical inputs must produce identical matrices"
        );
    }
    ...
    fn resolve_matrix_same_cost_higher_score_first() {
        ...
        assert_eq!(
            tier[0],
            at("api-high"),
            "higher score must sort first when cost is equal"
        );
    }
    ...
    fn resolve_matrix_same_cost_score_local_before_api() {
        ...
        assert_eq!(
            tier[0],
            lt("zero-local"),
            "local must precede API when cost and score tie"
        );
    }
    ...
    fn resolve_matrix_modelid_tiebreak_is_lexicographic() {
        let kind = SubtaskKind::TestGen;
        let available = vec![api_spec("zzz", 3.0, 15.0), api_spec("aaa", 3.0, 15.0)];
        let mut profiles = BTreeMap::new();
        ...
        assert_eq!(
            tier[0],
            at("aaa"),
            "lexicographically smaller id must come first"
        );
    ...
    fn default_profiles_covers_all_known_ids() {
        ...
        for id in expected_ids {
            assert!(
                profiles.contains_key(&mid(id)),
                "profile missing for '{id}'"
            );
        }
    }
    ...
    fn default_profiles_research_orderings_respected() {
        ...
        let pos = |id: &str| {
            debug_tier
                .iter()
                .position(|t| t.model.as_str() == id)
                .unwrap()
        };
        assert!(
            pos("gpt-5-high") < pos("opus"),
            "gpt-5-high must precede opus in Debugging tier"
        );
        ...
        let gemini_pos = lc_tier
            .iter()
            .position(|t| t.model.as_str() == "gemini-2.5-pro")
            .unwrap();
        assert_eq!(
            first_api,
            Some(gemini_pos),
    ...
    fn matrix_with_overrides_existing_tier() {
        let matrix = CapabilityMatrix::new()
            ...
            .with(
                ...
                vec![at("override-a"), at("override-b")],
            );
        assert_eq!(
            matrix.tier(SubtaskKind::Refactor),
            &[at("override-a"), at("override-b")]
        );
    }
