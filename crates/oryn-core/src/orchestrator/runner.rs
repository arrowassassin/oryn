...
impl ProcessRunner for SystemProcessRunner {
    fn run(&self, inv: &HarnessInvocation) -> Result<ProcessOutput, RunError> {
            ...
            && let Some(mut sink) = child.stdin.take()
        {
            ...
            sink.write_all(input.as_bytes()).map_err(|e| RunError::Io {
                program: inv.program.clone(),
                reason: e.to_string(),
            })?;
        }
...
pub fn parse_run(framework: AgentFramework, lines: &[String]) -> RunOutcome {
    match framework {
        AgentFramework::Aider | AgentFramework::Local => RunOutcome {
...
mod tests {
    ...
    fn parse_gemini_single_object_extracts_response_and_usage() {
        let out = parse_run(
            ...
            &lines(&[
                r#"{"response":"done","usage":{"prompt_tokens":500,"completion_tokens":120}}"#,
            ]),
        );
    ...
    fn parse_aider_joins_text_with_zero_usage() {
        let out = parse_run(
            ...
            &lines(&["Applied edit to auth.ts", "Commit a1b2c3"]),
        );
    ...
    fn parse_structured_falls_back_to_raw_text_on_non_json() {
        let out = parse_run(
            AgentFramework::ClaudeCode,
            &lines(&["not json at all", "still not"]),
        );
    ...
    fn parse_run_is_deterministic() {
        ...
        assert_eq!(
            parse_run(AgentFramework::ClaudeCode, &frames),
            parse_run(AgentFramework::ClaudeCode, &frames)
        );
    }
    ...
    impl ProcessRunner for FakeRunner {
        fn run(&self, inv: &HarnessInvocation) -> Result<ProcessOutput, RunError> {
            *self.seen.lock().unwrap() = Some(inv.clone());
            self.result.clone().map_err(|()| RunError::Spawn {
                program: inv.program.clone(),
                reason: "missing".into(),
            })
        }
    ...
    fn provider_spec_roundtrips_target() {
        ...
        assert_eq!(
            provider.spec().target(),
            ExecutionTarget::new(AgentFramework::Codex, ModelId::new("gpt-5.2"))
        );
    }
